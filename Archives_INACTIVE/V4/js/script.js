...
// Make PIXI globally available for other scripts
window.PIXI = PIXI;
...
const highlightButton = (button, container) => {
    if (!container) return;
...
const setNoContentMessage = (container, contentType) => {
    if (container) {
...
const createControlButton = (text, type, onClick) => {
...
const populateControls = (type, model) => {
...
const updateUIVisibility = (isLoading = false) => {
...
const updateModelFilenameDisplay = () => {
    if (!state.selectedModel) {
...
const updateSelectionOutline = (model) => {
...
const fitAndPositionNewModel = (model) => {
...
const loadModel = async (source) => {
    ...
    try {
        ...
        // A short delay to allow PIXI to calculate the model's bounds
...
const toggleHitAreaFramesVisibility = () => {
...
const triggerMotionForHitArea = (model, hitAreaName) => {
    const motionManager = model.internalModel?.motionManager;
...
const simulateTapOnHitArea = (model, hitAreaName, buttonElement) => {
...
const handleModelFilenameScroll = (event) => {
...
const initializeSelectionOutline = () => {
