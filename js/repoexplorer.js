...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderPreviewActions = (fileItem, url, isFallback = false) => {
        ...
        // Add an "Import Model" button for model files
    ...
    const displayFilePreview = async (fileItem) => {
        ...
        const savePreviewToCache = (path, type, content) => {
            try {
                sessionStorage.setItem(getPreviewCacheKey(path), JSON.stringify({ type, content, timestamp: Date.now() }));
            } catch (e) {
    ...
    const fetchGitHubContentsWithCache = async (path) => {
    ...
    const handleImportModel = (fileItem, sourceUrlOverride = null) => {
    ...
    const handleFeUrl = async () => {
        ...
        if (owner && repo) {
            ...
            await fetchAndDisplayContents(path);
            ...
            if (previewFile) {
                ...
                if (fileItemElement) {
                    const fileItem = { path: fullPath, name: previewFile, type: 'file' };
